...
func (client CycleTLS) Do(URL string, options Options, Method string) (Response, error) {
	...
	if ja3 == "RAND" {
		ja3 = "771,4866-4867-4865-49196-49200-159-52393-52392-52394-49195-49199-158-49188-49192-107-49187-49191-103-49162-49172-57-49161-49171-51-157-156-61-60-53-47-255,0-11-10-16-22-23-49-13-43-45-51-21,29-23-30-25-24-256-257-258-259-260,0-1-2"

		removable := []string{"-30-", "-25-", "-49-", "-256-", "-257-", "-258-", "-259-", "-103-", "-107-", "-156-", "-157-", "-158-", "-159-"}
		for _, r := range removable {
			if rr.Intn(2) == 0 {
				ja3 = strings.ReplaceAll(ja3, r, "-")
			}
		}

		removable = []string{"-21,", "-260,"}
		for _, r := range removable {
			if rr.Intn(2) == 0 {
				ja3 = strings.ReplaceAll(ja3, r, ",")
			}
	...
	if options.SignatureAlgorithms == "RAND" {
		...
		rr.Shuffle(len(sigAlgs), func(i, j int) {
			sigAlgs[i], sigAlgs[j] = sigAlgs[j], sigAlgs[i]
		})
		options.SignatureAlgorithms = strings.Join(sigAlgs, ",")
	}
